...
#Rome Italy
g = WeightedGraph(0)
g.load_graph("rome_italy (3).pkl")
r = 8430
#McD's at 8389, 8390, 8391, 8392, 8393, 8394, 8395, 8396

#time trial over 10 runs
average_time = 0
for i in range (10):
    start_time = time.perf_counter()
    T, d, path, total_distance = Dijkstras(g, r, dest=None)  # For example, find the path to node 5
    end_time = time.perf_counter()
    McDs_time = end_time - start_time
    average_time += McDs_time
    print("Time to run Dijkstra's on graph:", McDs_time)

print("Average time to run Dijkstra's on graph:", average_time/10)
